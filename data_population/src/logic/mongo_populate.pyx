...
cpdef generate_email(df):
    ...
    cdef int random_df_index = random.randint(0, len(df) - 1)
    cdef str suffix = EMAIL_DOMAIN_NAMES[random_index]
    cdef str prefix = ''.join(df.iloc[random_df_index])
...
cpdef generate_road_name(df):
    ...
    cdef str street_name = ' '.join(df.iloc[random_df_index])
...
cpdef generate_people():
    ...
    for i, filename in enumerate(os.listdir(ANIME_DIR)):
        if i % 2000 == 0:
            print(f"{datetime.now().time()} - MongoDB: {i} / {anime_dir_length} completed ...")
